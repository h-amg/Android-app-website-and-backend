...
class Email(db.Model):
    ...
    price_plan = db.Column(db.String)
    email_added_on = db.Column(db.DateTime, nullable=False)
    source = db.Column(db.String)
    form_data_as_json = db.Column(db.String)

    __table_args__ = (UniqueConstraint('email', 'price_plan', 'source', name='_email_source_unique_contraint'),)

    def __init__(self, email, price_plan, source=None, form_data_as_json=None):
        self.email = email
        self.price_plan = price_plan if price_plan else None
